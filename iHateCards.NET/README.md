...
- Раскладка карт на листе: A6/A5/A4/A3/SRA3 **и свой размер листа (50–5000 мм)**
  для широкоформатной печати, размер карты, блид, метки реза, полароид-рамка.
  Для больших листов разрешение растра понижается автоматически (лист 700 × 1000 мм —
  235 dpi вместо 300), иначе одна страница заняла бы под гигабайт памяти.
...
- Окно печати: параметры слева, предпросмотр листа справа; кнопка «Свойства» открывает
  системные настройки драйвера, настройки двухсторонней печати — в отдельном окне.
  На macOS файл/экспорт/печать/обновления живут в системной строке меню.
- Печать:
